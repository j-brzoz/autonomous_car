...
def main():
    ...
    model_yolov11 = YOLO("yolo11s.pt") #n, s, x
    # start_time = time.time()  # Zaczynamy liczyć czas
    # interval = 2  # Interwał zapisów w sekundach
    # frame_count = 620  # Licznik klatek

    #for video recording
    # save = True
    fps = 6000 / TIME_STEP
    duration = 10
    video_filename = "output_video_yolo11s_60fps_10s_no_hyperparams.mp4"
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Kodowanie wideo (MP4)
    ...
    while DRIVER.step() != -1 and frame_count < frames_to_capture:
        ...
        if i % int(TIME_STEP / DRIVER.getBasicTimeStep()) == 0:
            ...
            if HAS_CAMERA:
                ...
                results = model_yolov11.predict(image_rgb)
                # results = model_yolov11.predict(image_rgb, conf=0.4, iou=0.5)  # YOLOv11
                end_time = time.perf_counter()
                annotated_image = results[0].plot()  # Annotated image z YOLO
    ...
    if frame_count > 0:
        ...
        print(f"Średni czas przetwarzania: {average_processing_time:.3f} sekundy")
    else:
        print("Brak przetworzonych klatek.")
    del DRIVER
